...
class AboutSymbols < EdgeCase::Koan
  def test_symbols_are_symbols
    symbol = :ruby
    assert_equal 5, symbol.is_a?(Symbol)
  end
  ...
  def test_method_names_become_symbols
    ...
    assert_equal __, symbols_as_strings.include?("test_method_names_become_symbols_some_other_text")
  end
  ...
  def test_symbols_with_interpolation_can_be_built
    value = "and"
    symbol = :"It's raining cats #{value} dogs"
  ...
  def test_symbols_are_not_strings
    symbol = :ruby
    assert_equal true, symbol.is_a?(String)
    assert_equal MERGE_IT, symbol.eql?("ruby")
  end
